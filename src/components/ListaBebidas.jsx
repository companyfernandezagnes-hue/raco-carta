export default function ListaBebidas({ bebidas, onSeleccionar, modoVista = 'lista', favoritos = [], onToggleFavorito, comparador = [], onToggleComparador }) {
      if (bebidas.length === 0) return (
              <div style={{ padding: '60px 20px', textAlign: 'center', color: 'var(--raco-muted)', fontSize: '14px', letterSpacing: '0.08em' }}>
  ...
  if (modoVista === 'grid-sm' || modoVista === 'grid-lg') {
          ...
          return (
                    <div style={{ padding: '0 20px 20px' }}>
                        {destacados.length > 0 && (
                                  <>
                                              <p style={{ fontSize: '10px', letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--raco-muted)', marginBottom: '12px' }}>Seleccion del sumiller</p>
                        ...
                        {resto.length > 0 && (
                                  <>
                                      {destacados.length > 0 && <p style={{ fontSize: '10px', letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--raco-muted)', marginBottom: '12px' }}>Carta completa</p>}
...
function TarjetaGrid({ bebida, onSeleccionar, destacado, esPequena, esFavorito, onToggleFavorito, enComparador, onToggleComparador }) {
      return (
              <div style={{ position: 'relative', background: 'var(--raco-mid)', border: '1px solid ' + (destacado ? 'rgba(160,80,20,0.6)' : 'var(--raco-line)'), borderRadius: 'var(--radius)', overflow: 'hidden', cursor: 'pointer', transition: 'border-color 0.2s' }} onClick={() => onSeleccionar(bebida)} onMouseEnter={e => e.currentTarget.style.borderColor = 'rgba(160,80,20,0.6)'} onMouseLeave={e => e.currentTarget.style.borderColor = destacado ? 'rgba(160,80,20,0.6)' : 'var(--raco-line)'}>
                    <div style={{ width: '100%', aspectRatio: esPequena ? '2/3' : '3/4', background: 'var(--raco-deep)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                        {bebida.foto_url ? (
                    ...
                    <div style={{ padding: esPequena ? '6px' : '10px' }}>
                        {bebida.ecologico && !esPequena && <span style={{ fontSize: '8px', background: '#0a2010', border: '1px solid #16a34a', color: '#86efac', borderRadius: '3px', padding: '1px 4px', marginBottom: '4px', display: 'inline-block' }}>🌿</span>}
                            <p style={{ fontSize: esPequena ? '11px' : '13px', color: 'var(--raco-text)', lineHeight: '1.2', marginBottom: '3px', overflow: 'hidden', textOverflow: 'ellipsis', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>{bebida.nombre}</p>
                        {!esPequena && <p style={{ fontSize: '10px', color: 'var(--raco-muted)', marginBottom: '5px' }}>{bebida.bodega}</p>}
                        {bebida.precio_botella && <p style={{ fontSize: esPequena ? '11px' : '14px', color: 'var(--raco-orange)' }}>{bebida.precio_botella.toFixed(0)}€</p>}
                  ...
                  {onToggleComparador && (
                          <button onClick={e => { e.stopPropagation(); onToggleComparador(bebida) }} style={{ position: 'absolute', top: '5px', left: '5px', background: enComparador ? 'rgba(160,80,20,0.6)' : 'rgba(0,0,0,0.5)', border: '1px solid ' + (enComparador ? 'var(--raco-orange)' : 'transparent'), borderRadius: '4px', padding: '2px 5px', cursor: 'pointer', fontSize: '9px', color: enComparador ? 'var(--raco-orange)' : 'rgba(255,255,255,0.6)', letterSpacing: '0.05em' }}>
                                    ⚖
                          </button>
                    )}
              </div>
...
function TarjetaBebida({ bebida, onSeleccionar, destacado, esFavorito, onToggleFavorito, enComparador, onToggleComparador }) {
      const puntuaciones = Array.isArray(bebida.puntuaciones)
            ...
            return (
                    <div onClick={() => onSeleccionar(bebida)} style={{ background: 'var(--raco-mid)', border: '1px solid ' + (destacado ? 'rgba(160,80,20,0.6)' : 'var(--raco-line)'), borderRadius: 'var(--radius)', padding: '14px', cursor: 'pointer', transition: 'border-color 0.2s, background 0.2s', display: 'flex', gap: '12px', alignItems: 'center', position: 'relative' }} onMouseEnter={e => e.currentTarget.style.borderColor = 'rgba(160,80,20,0.6)'} onMouseLeave={e => e.currentTarget.style.borderColor = destacado ? 'rgba(160,80,20,0.6)' : 'var(--raco-line)'}>
                        {bebida.foto_url ? (
                                <div style={{ width: '56px', height: '72px', flexShrink: 0, borderRadius: '6px', overflow: 'hidden', background: 'var(--raco-deep)', border: '1px solid var(--border)' }}>
                                          <img src={bebida.foto_url} alt={bebida.nombre} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                                </div>
                              ) : (
                                <div style={{ width: '56px', height: '72px', flexShrink: 0, borderRadius: '6px', background: 'var(--raco-deep)', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '4px', flexWrap: 'wrap' }}>
                                      {destacado && (
                                    <span style={{ fontSize: '9px', letterSpacing: '0.15em', textTransform: 'uppercase', color: 'var(--raco-orange)', border: '1px solid var(--raco-orange)', borderRadius: '4px', padding: '2px 6px' }}>
                                                  Recomendado
                                    </span>
                                            )}
                                      {bebida.ecologico && (
                                  ...
                                  <h3 style={{ fontSize: '15px', fontWeight: 'normal', color: 'var(--raco-text)', marginBottom: '3px', lineHeight: '1.3', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                      {bebida.nombre}
                                  </h3>
                                  <p style={{ fontSize: '12px', color: 'var(--raco-dim)', marginBottom: '8px' }}>
                              ...
                              {bebida.precio_copa && <p style={{ fontSize: '12px', color: 'var(--raco-muted)' }}>copa {bebida.precio_copa.toFixed(0)} euros</p>}
                                  <div style={{ display: 'flex', gap: '4px', marginTop: '4px' }}>
                                    ...
                                    <button onClick={e => { e.stopPropagation(); onToggleFavorito(bebida) }} style={{ background: 'none', border: '1px solid ' + (esFavorito ? '#e53e3e' : 'var(--raco-line)'), borderRadius: '4px', padding: '3px 6px', cursor: 'pointer', fontSize: '13px', color: esFavorito ? '#e53e3e' : 'var(--raco-muted)', lineHeight: 1 }}>
                                      ...
                                      {onToggleComparador && (
                                    <button onClick={e => { e.stopPropagation(); onToggleComparador(bebida) }} style={{ background: enComparador ? 'rgba(160,80,20,0.6)' : 'none', border: '1px solid ' + (enComparador ? 'var(--raco-orange)' : 'var(--raco-line)'), borderRadius: '4px', padding: '3px 6px', cursor: 'pointer', fontSize: '11px', color: enComparador ? 'var(--raco-orange)' : 'var(--raco-muted)', lineHeight: 1 }}>
                                                  ⚖
                                    </button>
                                            )}
                                  </div>
...
function Tag({ children }) {
      return (
              <span style={{ fontSize: '11px', padding: '3px 8px', borderRadius: '4px', background: 'var(--raco-deep)', color: 'var(--raco-muted)', border: '1px solid var(--border)', textTransform: 'capitalize' }}>
                  {children}
              </span>
            )
}
