...
export default function PanelAdmin({ bebidas, onCerrar, onActualizar }) {
  ...
  function abrirEditar(b) {
    ...
    setForm({
      ...
      anada: b.anada || '',
      uvas: b.uvas || '',
      tipo_uva_secundaria: b.tipo_uva_secundaria || '',
      parcela: b.parcela || '',
      nota_cata: b.nota_cata || '',
      maridajes: Array.isArray(b.maridajes) ? b.maridajes.join(', ') : (b.maridajes || ''),
      temperatura: b.temperatura || '',
      graduacion: b.graduacion || '',
      precio_copa: b.precio_copa || '',
      precio_botella: b.precio_botella || '',
  ...
  const card = {
    background:'#1a1a1a',borderRadius:'12px',padding:'24px',width:'100%',
    maxWidth:'640px',maxHeight:'90vh',overflowY:'auto',color:'#fff'
  }
  const inp = {
    width:'100%',background:'#2a2a2a',border:'1px solid #444',borderRadius:'8px',
      ...
      <div style={card}>
        {fase === 'login' && (
          <>
            ...
            <input style={inp} type="password" placeholder="Contraseña" value={pass}
          ...
          <>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'16px'}}>
              ...
              <div style={{display:'flex',gap:'8px'}}>
                ...
                <button style={btn('#444')} onClick={onCerrar}>✕</button>
              </div>
            </div>
            {bebidas.map(b => (
        ...
        {fase === 'editando' && (
          <>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'16px'}}>
              <h2 style={{margin:0}}>{bebida ? '✏️ Editar' : '➕ Nueva bebida'}</h2>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0 16px'}}>
              {[
                ['Nombre *','nombre','text'],['Categoría *','categoria','text'],
                ['Subcategoría','subcategoria','text'],['Bodega','bodega','text'],
                ['Productor','productor','text'],['País','pais','text'],
                ['Región / D.O.','region','text'],['Añada','anada','number'],
                ['Uva principal','uvas','text'],['Uva secundaria','tipo_uva_secundaria','text'],
                ['Parcela','parcela','text'],['Temperatura','temperatura','text'],
                ['Graduación (%)','graduacion','number'],['Precio copa (€)','precio_copa','number'],
            ...
            <label style={label}>Notas IA (análisis automático)</label>
